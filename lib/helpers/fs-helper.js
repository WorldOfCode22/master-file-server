...
FSHelpers.checkTokenAndId = (cookieObj) => {
  let cookieToken = cookieObj.token
  ...
  return new Promise((resolve) => {
    ...
    rl.on('line', (line) => {
      ...
      if (token.userId === userId && token.token === cookieToken) resolve(true)
    })
    rl.on('close', () => {
      resolve(false)
    })
...
FSHelpers.getTemplate = async (template) => {
  let header = await new Promise((resolve, reject) => {
    fs.readFile(FSHelpers.templateFile + '/' + 'header' + '.html', (err, file) => {
      if (err) reject(new Error('Could Not Read File'))
      else resolve(file.toString())
    })
  })
  let footer = await new Promise((resolve, reject) => {
    fs.readFile(FSHelpers.templateFile + '/' + 'footer' + '.html', (err, file) => {
      if (err) reject(new Error('Could Not Read File'))
      else resolve(file.toString())
    })
  })
  return new Promise((resolve, reject) => {
    fs.readFile(FSHelpers.templateFile + '/' + template + '.html', (err, file) => {
      if (err) reject(new Error('Could Not Read File'))
      else {
        let finalFile = header + file.toString() + footer
        resolve(finalFile)
      }
    })
  })
